...
13. **Cold-start image pulls fail under parallel stampede.** Each runner pod gets its own ephemeral DinD with an empty image cache, so when N pods all run a workflow that references `services:` (or any `docker pull`) at the same time, they all hammer the upstream registry simultaneously. Under load on this single-node cluster, a normally-fast pull stretches to 2+ minutes, which exceeds typical service-container health-check windows (`--health-retries 10 × --health-interval 5s` = 50 s). The visible symptom is `##[error]One or more containers failed to start` even though the image *did* eventually pull. Mitigated by **prebaking** hot images into the runner: `build.sh` pulls them and saves tars to `k8s/runner-image/preloaded-images/`, the Dockerfile ships them to `/opt/preloaded-images/`, and a `preload-images` init container in each `k8s/values/*.yaml` loads them into DinD after the `dind` sidecar's startupProbe succeeds but before the runner accepts a job. To add another image, append it to `PRELOAD_IMAGES` in `build.sh`, bump `VERSION`, run `./build.sh`, and `helm upgrade` each scope.
14. **btrfs metadata ENOSPC takes down k3s.** The host's root filesystem is btrfs; `df` reports `Inodes: 0` because btrfs allocates inodes dynamically. Container layer storage in `/var/lib/rancher/k3s/agent/containerd/io.containerd.snapshotter.v1.overlayfs/` accumulates millions of small files (one incident reached **2.7M inodes**). Btrfs allocates raw device space in **block groups** split between data and metadata profiles; once the whole device is allocated and the metadata block groups fill, every `mkdir` returns `no space left on device` even though `df` shows tens of GiB of data-side headroom. k3s's startup probes mount transient overlayfs trees and fail with this error, putting k3s into a `activating` loop indefinitely. Diagnose with `sudo btrfs filesystem df /` (look for `Metadata,DUP` near 100% and `Device unallocated: 1 MiB`). Recovery: stop k3s, `rm -rf /var/lib/rancher/k3s/agent/containerd/io.containerd.snapshotter.v1.overlayfs` and `io.containerd.content.v1.content`, then `sudo btrfs balance start -dusage=10 /` to compact data block groups and release raw space for metadata. Mitigated by a weekly `btrfs-balance.timer` on the host (see prevention notes), tighter k3s image GC (`image-gc-high-threshold=90`), and a `check-btrfs-metadata` init container on every runner pod that refuses to start at ≥95% metadata usage (it reads `/sys/fs/btrfs/<uuid>/allocation/metadata/{total,used}_bytes` via a `hostPath` mount).
